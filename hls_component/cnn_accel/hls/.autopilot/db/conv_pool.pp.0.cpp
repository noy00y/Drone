...
# 8 "../hls_files/conv_pool.cpp" 2
# 23 "../hls_files/conv_pool.cpp"
...
#pragma HLS UNROLL
 acc_t sum = 0;
        VITIS_LOOP_58_1: for (int c=0; c<3; ++c)
        VITIS_LOOP_59_2: for (int i=0; i<3; ++i)
        VITIS_LOOP_60_3: for (int j=0; j<3; ++j) {
#pragma HLS UNROLL
...
#pragma HLS BIND_OP variable=prod op=mul impl=dsp latency=-1
 sum += prod;
    ...
    VITIS_LOOP_70_4: for (int f=0; f<4; ++f) {
#pragma HLS UNROLL
 acc_t tmp = partial[f] + b[f];
        out[f] = (tmp < (pix_t)0) ? (pix_t)0 : (pix_t)tmp;
    }
}
...
__attribute__((sdx_kernel("cnn_accel", 0))) void cnn_accel(
        ...
        const pix_t weights[8][3][3][3],
        const pix_t bias [8])
{
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=cnn_accel
# 87 "../hls_files/conv_pool.cpp"
...
#pragma HLS STREAM variable=fmap_out1 type=fifo depth=32
 ...
 static pix_t LB0[3][224];
    static pix_t LB1[3][224];
...
#pragma HLS ARRAY_PARTITION complete variable=lb_out
 VITIS_LOOP_142_1: for (int ch=0; ch<3; ++ch) {
#pragma HLS UNROLL
 ...
 pix_t new_pix =
            ...
            VITIS_LOOP_163_2: for (int ch=0; ch<3; ++ch) {
#pragma HLS UNROLL

 VITIS_LOOP_166_3: for (int k=3 -1; k>0; --k) {
#pragma HLS UNROLL
 A[ch][k] = A[ch][k-1];
                    B[ch][k] = B[ch][k-1];
                    C[ch][k] = C[ch][k-1];
...
#pragma HLS ARRAY_PARTITION variable=win.dat complete dim=0
 VITIS_LOOP_192_4: for (int ch=0; ch<3; ++ch)
                VITIS_LOOP_193_5: for (int i=0; i<3; ++i) {
#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION complete variable=out_grp
 ...
 if (!toggle) {
                    conv4_engine(win,
                                 (const pix_t (*)[3][3][3])&(weights[0]),
                                 &(bias[0]),
                                 out_grp);
                    VITIS_LOOP_214_6: for (int f=0; f<4; ++f) {
#pragma HLS UNROLL
 fmap_out0.write(out_grp[f]);
                    }
                } else {
                    conv4_engine(win,
                                 (const pix_t (*)[3][3][3])&(weights[4]),
                                 &(bias[4]),
                                 out_grp);
                    VITIS_LOOP_223_7: for (int f=0; f<4; ++f) {
#pragma HLS UNROLL
 fmap_out1.write(out_grp[f]);
                    }
                }
            }
        }
