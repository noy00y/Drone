...
extern "C" void cnn_accel(
    const data_t *img_in,
    const data_t *weights1, const data_t *B1,
    const data_t *weights2, const data_t *B2,
...
int main() {
    ...
    static data_t weights1[M1*C0*K*K], bias1[M1];
    static data_t weights2[M2*M1*K*K], bias2[M2];
    ...
    for (int i = 0; i < M1*C0*K*K;   i++) weights1[i]       = 0.1f;
    for (int i = 0; i < M1;          i++) bias1[i]       = 0.0f;
    for (int i = 0; i < M2*M1*K*K;   i++) weights2[i]       = 0.1f;
    for (int i = 0; i < M2;          i++) bias2[i]       = 0.0f;
    ...
    // Call HLS kernel
    cnn_accel(img_in, weights1, bias1, weights2, bias2, FC1_W, FC1_B, FC2_W, FC2_B, flag_out, 0);
